/*  patchmii_core -- low-level functions to handle the downloading, patching
	...
	net_init retry code (c) 2009 kerframil
...
void patchmii_network_init(void) {
	...
	printf("(*) Loading Network..."); fflush(stdout);
	do {
		while (1) {
			retval = net_init ();
			if (retval < 0) {
				if (retval != -EAGAIN) {
					debug_printf ("net_init failed: %d", retval);
					retry--;
					if (retry) {
						printf ("..retrying..");
						break;
					} else {
						debug_printf ("I need a network to download IOS, sorry :(");
						exit(0);
					}
				}
			}
